...
const DATABASE_URL =
  ...
  'postgresql://neondb_owner:npg_grldnN9hjkc7@ep-dark-poetry-az6rk6ec-pooler.c-3.ap-southeast-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require';
...
async function initDatabaseSchema() {
  try {
    ...
    // Convert id and role columns if existing table used uuid or enum types
    try {
    ...
    // Ensure columns in tasks table do not fail on null or missing constraints
    try {
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // Check bcrypt hash OR fallback direct string match
    let isValid = false;
...
app.post('/api/users', async (req, res) => {
  try {
    ...
    const userId = id && id.length > 20 ? id : crypto.randomUUID();
    const passwordHash = await bcrypt.hash(password || 'advrix123', 10);
  ...
  try {
    ...
    if (isCheckedIn !== undefined) {
      await pool.query('UPDATE users SET is_checked_in = $1, check_in_time = $2 WHERE id = $3', [
        isCheckedIn,
        checkInTime || (isCheckedIn ? '09:00 AM' : undefined),
        id,
      ]);
    }
